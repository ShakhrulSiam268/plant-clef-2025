...
# ---------------- CONFIG ----------------
IMAGE_PATH = "sample_data/RNNB-4-2-20240118.jpg"
# IMAGE_PATH = "sample_data/LISAH-BOU-0-65-20230512.jpg"
...
def main():
    ...
    # Check files
    ensure_file(IMAGE_PATH, "Set IMAGE_PATH to your garden image.")
    ...
    for (cx, cy, bw, bh) in boxes.tolist():
        ...
        x2 = (cx + bw / 2.0) * W
        y2 = (cy + bh / 2.0) * H
        if abs(x1 - x2) * abs(y1 - y2) < W * H * 0.3:
    ...
    with open(os.path.join(OUT_DIR, "instances.json"), "w") as f:
        json.dump(results, f, indent=2)

    # ---- Save MASK OVERLAY (tinted) ----
    overlay = orig_rgb.copy()
    for i, m in enumerate(instance_masks):
        c = np.array(((37*i) % 255, (91*i) % 255, (173*i) % 255), dtype=np.uint8)
        overlay[m] = (0.4 * overlay[m] + 0.6 * c).astype(np.uint8)

    overlay = cv2.resize(overlay, (500, 500))
    cv2.imwrite(os.path.join(OUT_DIR, "overlay_masks.png"),
                cv2.cvtColor(overlay, cv2.COLOR_RGB2BGR))

    # ---- Save BBOXES ON ORIGINAL (NO mask tint) ----
    bboxes_only = orig_rgb.copy()
    for i, box in enumerate(instance_boxes):
        x1, y1, x2, y2 = box.astype(int).tolist()
        cv2.rectangle(bboxes_only, (x1, y1), (x2, y2), (0, 255, 0), 10)
        cv2.putText(
            bboxes_only, str(i),
            (x1, max(0, y1 - 5)),
            cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 10
        )

    bboxes_only = cv2.resize(bboxes_only, (500, 500))
    cv2.imwrite(os.path.join(OUT_DIR, "bboxes_on_original.png"),
                cv2.cvtColor(bboxes_only, cv2.COLOR_RGB2BGR))

    print("Saved outputs in:", OUT_DIR)
    print("-", os.path.join(OUT_DIR, "bboxes_on_original.png"))
    print("-", os.path.join(OUT_DIR, "overlay_masks.png"))
    print("-", os.path.join(OUT_DIR, "instances.json"))
